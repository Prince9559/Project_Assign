...
import PricingPlanCard from "../../../components/jobs/PricingPlanCard";
...
const SubscriptionPricingPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [subscriptionRes, creditRes] = await Promise.all([
          ...
          axios.get(`${BASE_URL}/subscriptions/credits/plans`, {
        ...
        if (subscriptionRes.data.success) {
          const sortedSubs = [...subscriptionRes.data.plans].sort((a, b) => {
            const priceA =
          ...
          setSubscriptionPlans(sortedSubs);
        }

        // Handle credit packs
        if (creditRes.data.success) {
          const sortedCredits = [...creditRes.data.plans].sort(
            (a, b) => a.price - b.price,
          );
          setCreditPacks(sortedCredits);
        }
      } catch (err) {
  ...
  const buyCredits = async (pack) => {
    if (!razorpayLoaded) {
      alert("Payment system is initializing...");
      return;
    }
    ...
    try {
      ...
      const options = {
        ...
        handler: (response) => {
          //  SUCCESS — redirect EXACTLY as in original CreditPacksPage
          window.location.href = `/recruiter/credits/success?order_id=${orderData.order_id}&credits=${pack.credits}`;
        },
        prefill: {
  ...
  return (
    ...
    >
      <div className="bg-white">
            ...
            {subscriptionPlans.map((plan) => {
              ...
              return (
                ...
                <div
                  ...
                  className={`relative rounded-xl border p-6 transition-all flex flex-col ${
                    plan.is_featured
                      ? "ring-2 ring-[#1DB32F] bg-gradient-to-b from-[#9bc87c]/10 to-white border-[#1DB32F]"
                      : "bg-white border-gray-200 hover:shadow-md"
                  }`}
                >
                  {plan.is_featured && (
                    ...
                    //   MOST POPULAR
                    // </div>
                    <div className="absolute top-0 right-0 px-3 py-1 text-xs font-bold text-[#1DB32F] bg-white border border-[#1DB32F] rounded-bl-lg">
                      MOST POPULAR
                    </div>
                  )}
                  ...
                  <div className="text-center">
                    ...
                    {/* <button
                      ...
                      className={`w-full py-2 px-4 rounded-lg font-medium ${
                        isCreditProcessing && selectedCreditPack?.id === pack.id
        ...
        {/* Final CTA */}
        <div className="py-12 bg-[#9bc87c] text-[#1e1e2d]">
