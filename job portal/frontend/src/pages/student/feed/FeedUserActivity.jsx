...
export default function FeedUserActivity() {
  const { token, user } = useSelector((state) => state.auth);
  let userId;
  const location = useLocation();
  const stateUserId = location.state?.userId;
  console.log("the user id coming from the state is ", stateUserId);
  if(!stateUserId){userId=user?.id}
  else{userId=stateUserId;
    console.log("in the else",userId);
  }
  console.log("the userID now is ", userId);
    ...
    <MainLayout>
      <div className="flex justify-center min-h-screen px-2 bg-gray-100 sm:px-4 md:px-6 lg:px-8">
        ...
        <section className="w-full max-w-[750px] p-2 mx-auto">
          {/* Create post card not required on this page
