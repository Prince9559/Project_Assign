...
const FeedYourExperience = () => {
  ...
  const handleUploadProof = async (entry) => {
    try {
      const response = await userDetailsApi.uploadExperienceProof(
        entry.id,
        proofFiles[entry.id] || null,
        token
      );
      if (response?.success) {
        alert("Proof uploaded and re-approval requested.");
        const result = await userDetailsApi.getUserDetails(user.id);
        const experiences = result?.data?.userDetail?.experiences || [];
        setExperienceEntries((prev) =>
          prev.map((item) => {
            const found = experiences.find((x) => x.id === item.id);
            return found
              ? {
                  ...item,
                  approval_status: found.approval_status || "approved",
                  removed_by_company: !!found.removed_by_company,
          ...
          <div className="flex flex-col w-full bg-white rounded">
            ...
            <div className="px-4 pb-4 space-y-4 sm:px-6 sm:pb-6">
              {experienceEntries.map((entry) => (
                <div
                  key={entry.id}
                  className={`${isRejectedEntry(entry) ? "bg-red-50" : entry.bgColor} border-2 ${isRejectedEntry(entry) ? "border-red-200" : "border-blue-200"} rounded-lg p-4 sm:p-6`}
                >
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={isRejectedEntry(entry)}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <select
                            ...
                            disabled={entry.is_present || isRejectedEntry(entry)}
                    ...
                    {isRejectedEntry(entry) && (
                      <div className="p-3 border border-red-200 bg-red-100 rounded">
                        ...
                        <p className="text-sm text-red-700 mb-3">
                          Your experience entry has been rejected by company. Upload proof to request re-approval.
                        </p>
                        <div className="flex flex-wrap gap-2 items-center">
                          <input
                            ...
                            onChange={(e) =>
                              setProofFiles((prev) => ({
                                ...prev,
                                [entry.id]: e.target.files?.[0] || null,
                              }))
                            }
                            className="text-sm"
                          />
