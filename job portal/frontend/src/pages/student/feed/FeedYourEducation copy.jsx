...
const educationEntrySchema = z.object({
  ...
  level: z.enum(["school", "bachelors", "masters"]),
  ...
  education_certificate: z.string().optional(),
  ...
  if (data.level === "school" && !SCHOOL_STANDARDS.includes(data.standard_or_grade)) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      ...
      prev.map((e) =>
        e.id === id
          ? {
              ...
              course_id: null,
              specialization_id: null,
              standard_or_grade: level === "school" ? "Class XII" : e.standard_or_grade,
            }
          : e
      )
    );
            ...
            <div className="px-4 pb-4 space-y-3 sm:space-y-4 sm:px-6 sm:pb-6">
              {educationEntries.map((entry) => {
                const specializations =
                  entry.course_id && courses
                    ? getSpecializationsForCourse(entry.course_id) || []
                    : [];
                ...
                return (
                  ...
                  >
                    ...
                    {/* Institution */}
                    <div className="mb-3">
                      <label className="block mb-1 text-sm font-medium text-gray-700">Institution</label>
                      <Select
                        options={institutionOptions}
                        getOptionLabel={(opt) => opt.name}
                        getOptionValue={(opt) => opt.id}
                        value={
                          entry.school_college_id === null
                            ? OTHER_OPTION
                            : schoolColleges?.find((inst) => inst.id === entry.school_college_id) || null
                        }
                      ...
                      {entry.school_college_id === null && (
                        <Input
                          ...
                          onChange={(e) => handleInputChange(entry.id, "other_institution_name", e.target.value)}
                          className="mt-2"
                        />
                      )}
                      ...
                      <>
                        <div className="mb-3">
                          ...
                          <Select
                            ...
                            value={courses?.find((c) => c.id === entry.course_id) || null}
                        ...
                        {entry.course_id && specializations.length > 0 && (
                          <div className="mb-3">
                            ...
                            <Select
                              ...
                              getOptionLabel={(opt) => opt.name}
                              getOptionValue={(opt) => String(opt.id)}
                              value={specializations.find((s) => s.id === entry.specialization_id) || null}
                              onChange={(opt) => handleSpecializationChange(entry.id, opt)}
                              isClearable
                              isSearchable
                              className="text-sm"
                              classNamePrefix="select"
