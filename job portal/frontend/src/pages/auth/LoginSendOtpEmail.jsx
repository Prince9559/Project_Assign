...
export default function LoginSendOtpEmail() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      navigate("/login-verify-otp-email", { state: { email: data.email } });
    } catch (error) {
      if (
        error.response &&
        error.response.data &&
    ...
    >
      <div className="flex-1 w-full min-h-[14rem] flex justify-center">
        <form
          ...
          className="w-full max-w-xs p-2 bg-white rounded-lg shadow-md sm:p-3 md:p-4 sm:max-w-sm md:max-w-md"
        >
